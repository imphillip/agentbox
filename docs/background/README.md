...
Long-form essays from the ongoing agentbox exploration. They are dated arguments, not continuously updated accounts of the agent ecosystem.

The earlier `agentbox.id` mailbox service has ended, and the Soul Store has become independent. Product claims inside historical essays describe the context in which they were written unless an editorial note says otherwise; newer essays continue the inquiry beyond that product phase.
...
- **[From mailbox to directory to agent-first](from-mailbox-to-agent-first.md)** - a reconstruction of the mailbox product's shift from human signup, to Guardian-managed agents, to an agent-first flow that the control plane could not yet complete.
